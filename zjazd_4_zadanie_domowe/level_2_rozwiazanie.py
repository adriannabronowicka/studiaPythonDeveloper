...
def poorness_check(balance: int, money_to_pay: int):        #dodanie try i except ze zdefiniowanym wyjątkiem
    try:
        ...
        print(f"You don't have enough money to pay: {money_to_pay=}, {balance=}")
    else:
...
class CoolBankAccount(Account):
    ...
    def transfer(self, account: Account, money: int):
        ...
        account.receive(money)      #zmiana z self.receive na account.receive
        self.balance -= money
